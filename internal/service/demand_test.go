...
func TestDemandLifecycle(t *testing.T) {
	...
	d, err := svc.Create(ctx, admin, "新功能", "描述", 0, nil, false, nil, "")
	if err != nil {
		t.Fatalf("创建失败: %v", err)
	}
...
func TestDemandCreateWithoutEstimate(t *testing.T) {
	...
	d, err := svc.Create(ctx, admin, "新需求", "描述", 0, nil, false, nil, "")
	if err != nil {
		t.Fatalf("创建失败: %v", err)
	}
	...
	if _, err = svc.Create(ctx, admin, "", "", 0, nil, false, nil, ""); err == nil {
		t.Error("空标题应拒绝")
	}
}
...
func TestDemandUpdateOnlyTitleDescription(t *testing.T) {
	...
	d, _ := svc.Create(ctx, admin, "需求", "旧描述", 0, nil, false, nil, "")
	planned := time.Date(2026, 9, 1, 0, 0, 0, 0, time.Local)
...
func TestDemandCreateWithEstimate(t *testing.T) {
	...
	planned := time.Date(2026, 9, 1, 0, 0, 0, 0, time.Local)
	d, err := svc.Create(ctx, admin, "带预估", "", 4, &planned, false, nil, "")
	if err != nil {
	...
	d2, err := svc.Create(ctx, admin, "创建即确认", "", 6, nil, true, nil, "")
	if err != nil {
	...
	// 勾选已确认但人天为 0 → 拒绝
	if _, err = svc.Create(ctx, admin, "缺人天", "", 0, nil, true, nil, ""); err == nil {
		t.Error("已确认但人天为 0 应拒绝")
	}

	// 人天为负 → 拒绝
	if _, err = svc.Create(ctx, admin, "负人天", "", -2, nil, false, nil, ""); err == nil {
		t.Error("负人天应拒绝")
	}

	// 不带预估 → 保持 draft，行为与现状一致
	d3, err := svc.Create(ctx, admin, "普通创建", "", 0, nil, false, nil, "")
	if err != nil {
