...
// The DemandFunc type is an adapter to allow the use of ordinary
// function as Demand mutator.
type DemandFunc func(context.Context, *ent.DemandMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DemandFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DemandMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DemandMutation", m)
}
